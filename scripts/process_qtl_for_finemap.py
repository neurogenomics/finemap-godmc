...
print(f"Proportion of CpGs with at least one significant SNP: {data.groupby('cpg')['significant'].first().sum() / len(data['cpg'].unique()):.4%}")
...
cols_to_drop = ['hetisq', 'hetchisq', 'hetpval', 'tausq', 'beta_are_a1', 'se_are',
                'pval_are', 'se_mre', 'pval_mre']
